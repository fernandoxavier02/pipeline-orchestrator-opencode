...
| W7.2 DONE | `src/opencode/stop-hook.cjs` | `.claude/hooks/stop-hook.cjs` (842) | hook `session.idle` observer | B | append `.pipeline/run-log.jsonl`; cria/preserva `fidelity-report.json`; deduplica linhas materialmente iguais; plugin local/global carrega via hooks `event`/`session.idle`; `node tests/unit/stop-hook.test.cjs` e `npm test` passam. Limite: observer-only, sem paridade determinística de Stop. |
| W7.3 DONE | `src/opencode/session-cleanup-hook.cjs` | `.claude/hooks/session-cleanup-hook.cjs` (116) | hook `session.idle` observer | B | remove exec-windows da sessão atual; remove locks expirados; marca lock como `completed` só sem run governado ativo ou com run terminal; protege contra fuga por symlink/junction e temp precriado; plugin local/global carrega via hooks `event`/`session.idle`; `node tests/unit/session-cleanup-hook.test.cjs` e `npm test` passam. Limite: cleanup best-effort observer-only. |
| W7.4 DONE | `src/opencode/compaction-bridge.cjs` | (novo — não existe no canônico) | hook `experimental.session.compacting` | Bônus | injeta contexto seguro em `output.context` com runId, fase atual e blocos pendentes como JSON inerte; preserva contexto existente; plugin local/global expõe `experimental.session.compacting`; instalador global não gera plugin vazio; `node tests/unit/compaction-bridge.test.cjs`, `node tests/unit/global-install.test.cjs` e `npm test` passam. Limite: preserva contexto, não força gate. |
...
| # | Slice | Origem canônica | Alvo porta | Aceitação |
|---|---|---|---|---|
| W9.1 DONE | Expandir 9 agentes existentes | `agents/core/*.md` + `agents/executor/*.md` + `agents/quality/*.md` | prompts detalhados com step-by-step + gate protocol wiring; cada agente ≥ 50 linhas; testes passam; evidência em `tmp/w9-1-agent-expansion-evidence.md` |
| W9.2 DONE | Portar agentes type-specific faltantes | `agents/executor/type-specific/*.md` (21 arquivos: bugfix-diagnostic, root-cause-analyzer, regression-tester, feature-implementer, integration-validator, slice-planner, audit-intake, domain-analyzer, compliance-checker, risk-matrix-generator, ux-simulator, qa-validator, accessibility-auditor, spec-format-gate, content-reviewer, adversarial-critic, post-impl-validator, adversarial-review-coordinator, adversarial-architecture-critic, adversarial-quality-reviewer, adversarial-security-scanner) | 21 novos agentes em `.opencode/agents/`; testes passam; evidência em `tmp/w9-2-type-specific-agents-evidence.md` |
...
| W10.2 DONE | `src/lib/step-1-7-routing.cjs` | `lib/step-1-7-routing.cjs` (275) | porta local com contrato de gate OpenCode | `appendStep17Routing`, `branchToCanonical`, `BRANCH_VALUES`; 95 testes passam; evidência em `tmp/w10-2-step-1-7-routing-evidence.md` |
