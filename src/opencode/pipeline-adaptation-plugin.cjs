...
const { createSkillFrontmatterParserHooks } = require('./skill-frontmatter-parser.cjs');
...
function createPipelineAdaptationHooks(ctx = {}, options = {}) {
  ...
  return mergeHooks(
    ...
    createDispatchGuardHooks({ ...options, projectDir }),
    ...
    createSkillFrontmatterParserHooks({ ...options, projectDir }),
    ...
    createCompactionBridgeHooks({ ...options, projectDir }),
  );
}
